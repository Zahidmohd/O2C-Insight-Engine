 ...
 *
 * Usage: npx ts-node tests/smokeTest.ts
...
function post(query: string): Promise<TestResponse> {
...
interface TestCase {
    name: string;
    query: string;
    validate: (res: TestResponse) => void;
}
...
async function runTests(): Promise<void> {
    ...
    console.log('  SMOKE TEST SUITE — O2C Insight Engine (NestJS)');
    console.log('='.repeat(60));
    console.log();
