...
const ALLOWED_DATA_EXTENSIONS = ['.csv', '.jsonl', '.json'];
const SKIP_PATTERNS = ['__MACOSX', '.DS_Store', 'Thumbs.db'];

/**
 * Extracts supported data files from a ZIP buffer or file path.
 * @param {Buffer|string} zipInput - ZIP file buffer (from multer) or file path
...
function extractZip(zipInput) {
    ...
    for (const entry of entries) {
        ...
        if (SKIP_PATTERNS.some(p => entryPath.includes(p))) continue;
