...
const db: any = require('../db/connection');

const USE_TURSO_VECTOR: boolean = process.env.USE_TURSO_VECTOR !== 'false';

// ─── Table Initialization ────────────────────────────────────────────────────

async function initDocumentTables(dbConn: any = db): Promise<void> {
    const isTurso: boolean = dbConn.type === 'turso';
...
async function insertDocument(dbConn: any = db, { title, filename, fileType, fileSize }: InsertDocumentParams): Promise<any> {
...
async function insertChunks(dbConn: any = db, documentId: number, chunks: ChunkInput[]): Promise<void> {
    const isTurso: boolean = dbConn.type === 'turso' && USE_TURSO_VECTOR;

    if (isTurso) {
        // Turso: use individual execute() calls with vector32() — batch() doesn't support vector32()
        for (const chunk of chunks) {
            const embStr = '[' + Array.from(chunk.embedding).map((v: any) => Number(v).toFixed(6)).join(',') + ']';
        ...
        const statements = chunks.map((chunk: ChunkInput) => ({
...
async function deleteDocument(dbConn: any = db, id: number): Promise<void> {
...
async function listDocuments(dbConn: any = db): Promise<any[]> {
...
async function getChunkCount(dbConn: any = db): Promise<number> {
 ...
 */
async function tursoVectorSearch(dbConn: any, queryEmbedding: number[], topK: number = 5): Promise<SearchResult[]> {
    const embStr = '[' + Array.from(queryEmbedding).map((v: number) => v.toFixed(6)).join(',') + ']';
 ...
 */
async function inMemorySearch(dbConn: any, queryEmbedding: number[], topK: number = 5, threshold: number = 0.3): Promise<SearchResult[]> {
 ...
 */
async function searchSimilar(dbConn: any = db, queryEmbedding: number[], topK: number = 5, threshold: number = 0.3): Promise<SearchResult[]> {
    const isTurso: boolean = dbConn.type === 'turso' && USE_TURSO_VECTOR;
