...
function addNode(state, id, type, label, row) {
...
function addEdge(state, source, target, type) {
...
function extractGraphGeneric(rows, config) {
    const state = createGraphState();
    ...
    const rels = (config.relationships || []).map(r => {
    ...
    const nodeColumns = new Map(); // tableName → Set of column names
    ...
    for (const row of rows) {
        ...
        for (const rel of rels) {
            ...
            const fromVals = rel.from.cols.map(c => row[c]);
            const toVals = rel.to.cols.map(c => row[c]);

            // Skip if any value is missing
            if (fromVals.some(v => !valid(v)) || toVals.some(v => !valid(v))) continue;
 ...
 * @param {object|null} config - Active dataset config (null = O2C default)
 * @returns {{ nodes, edges, graphTruncated }}
 */
function extractGraph(rows, config = null) {
