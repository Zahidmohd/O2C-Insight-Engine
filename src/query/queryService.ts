...
import { getSqlFromLLM, generateNLAnswer } from './llmClient';
import { validateSql } from './validator';
import { executeQuery } from './sqlExecutor';
import { extractGraph } from './graphExtractor';
 ...
 * Imported from cache/queryCache.js for centralized cache management.
 */
const { getCached, setCached, clearTenantCache, normalizeQuery } = require('../cache/queryCache');

/**
 * Validates domain safety before spending API tokens
 * Rejects questions wildly outside the active dataset's domain
 */
function isIntentValid(query: string): { valid: boolean; message?: string } {
    ...
    const words: string[] = queryLower.match(/\b\w+\b/g) || [];
...
function isDomainQuery(query: string): { valid: boolean; message?: string } {
    const queryLower = query.toLowerCase();

    // Domain keywords sourced from active dataset config
    const matchedKeywords = getActiveConfig().domainKeywords.filter((kw: string) => queryLower.includes(kw));
 ...
 */
async function tryRuleBasedQuery(query: string, tag: string, options: any): Promise<any> {
    ...
    const { executeQueryDirect } = require('./sqlExecutor');
    const result = await executeQueryDirect(sql);
    ...
    return {
        ...
        nlAnswer: result.rowCount > 0
            ? `Found ${result.rowCount} ${(config.tables.find((t: any) => t.name === matchedTable.name)?.displayName || matchedTable.name.replace(/_/g, ' '))} record${result.rowCount !== 1 ? 's' : ''}.`
            : null,
        explanation,
 ...
 */
function extractHighlightNodes(sql: string): string[] {
    const config = getActiveConfig();
...
function extractHighlightNodesO2C(sql: string): string[] {
...
function extractHighlightNodesGeneric(sql: string, config: any): string[] {
    const highlights: string[] = [];
    if (!sql) return highlights;
 ...
 */
function buildGenericIdChecks(config: any): any[] {
    const checks: any[] = [];
    for (const t of config.tables) {
        if (!t.primaryKey || t.primaryKey.length !== 1) continue;
        const pk = t.primaryKey[0];
 ...
 */
function formatResponse(result: any, rawSql?: string): any {
    ...
    const highlightNodes = extractHighlightNodes(rawSql as string);
 ...
 */
function buildExplanation(query: string, sql: string): any {
    ...
    const entitiesFromQuery: string[] = (getActiveConfig().entities || []).filter((e: string) => lower.includes(e.toLowerCase()));

    // Also infer entities from tables referenced in the generated SQL
    const sqlLower = (sql || '').toLowerCase();
    const config = getActiveConfig();
    const tableEntityMap: { table: string; entity: string }[] = config.name === 'sap_o2c'
        ? [
 ...
 */
function buildQueryPlan(sql: string, explanation: any): any {
    const config = getActiveConfig();
    if (!sql) {
    ...
    const sqlHasTable = (tableName: string) => new RegExp(`\\b${tableName}\\b`, 'i').test(sql);

    // 1. Extract tables referenced in SQL (FROM + JOIN clauses)
    const tablesUsed = config.tables
        .filter((t: any) => sqlHasTable(t.name))
    ...
    const reasoning = joinPath.length > 0
        ? `Traversed ${joinPath.length} relationship(s) across ${tablesUsed.length} table(s): ${joinPath.map((j: any) => j.label).join(' \u2192 ')}`
 ...
 */
function calculateConfidence({ queryType, fallbackApplied, rowCount, isAggregation }: { queryType: string; fallbackApplied: boolean; rowCount: number; isAggregation: boolean }): { score: number; label: string; reasons: string[] } {
 ...
 */
function tryFallbackSql(query: string): string | null {
    const config = getActiveConfig();
    const lower = query.toLowerCase();
 ...
 */
function buildSuggestedQueries(): string[] {
    const config = getActiveConfig();
    ...
    const patterns = [
        (name: string) => `Show all ${name}`,
        (name: string) => `Count all ${name}`,
        (name: string) => `List ${name} with details`,
    ];
 ...
 */
async function processQuery(naturalLanguageQuery: string, requestId: string = 'dev-local', options: any = {}, dbConn: any = null, configOverride: any = null): Promise<any> {
    ...
    const cached = await getCached(naturalLanguageQuery, options.includeSql, tenantId, activeConfig);
    if (cached) {
    ...
    if (ruleBasedResult) {
        ...
        await setCached(naturalLanguageQuery, options.includeSql, ruleBasedResult, tenantId, activeConfig);
    ...
    if (llmFailed) {
        ...
        if (fallbackSql) {
            ...
            try {
                ...
                if (fbResult.success && fbResult.rowCount > 0) {
                    ...
                    const response: any = formatResponse(fbResult, options.includeSql ? fallbackSql : undefined);
                    ...
                    await setCached(naturalLanguageQuery, options.includeSql, response, tenantId, activeConfig);
                    return response;
                }
            } catch (fbErr: any) {
        ...
        await setCached(naturalLanguageQuery, options.includeSql, fallbackResponse, tenantId, activeConfig);
    ...
    try {
        ...
        const idChecks: any[] = activeConfig.name === 'sap_o2c'
            ? [
        ...
        for (const check of idChecks) {
            ...
            if (match && match[1]) {
                ...
                if (checkResult.success && checkResult.rowCount === 0) {
                    ...
                    const suggestions = sampleResult.success ? sampleResult.rows.map((r: any) => r[check.column]) : [];
        ...
        if (dbResult.rowCount === 0) {
            ...
            const relaxationTables: string[] = activeConfig.name === 'sap_o2c'
                ...
                : activeConfig.tables.map((t: any) => t.name);
        ...
        if (dbResult.rowCount === 0) {
            ...
            let resultStatus: string, emptySummary: string, reason: string;
        ...
        if (isAggregation) {
            ...
            const aggNodes = dbResult.rows.map((row: any, i: number) => {
        ...
        if (finalResponse.success) await setCached(naturalLanguageQuery, options.includeSql, finalResponse, tenantId, activeConfig);
...
function clearCache(tenantId: string | null = null): void {
    if (!tenantId) {
        responseCache.clear();
