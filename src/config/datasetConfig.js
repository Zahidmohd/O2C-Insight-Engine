 ...
 * Backward-compatible: flat exports (domainKeywords, entities, relationships) preserved.
 */
...
const datasetConfig = {
    ...
    // ─── Validated Join Relationships ────────────────────────────────────
    // Encode the exact join paths the LLM must follow.
    // `from` / `to` use "table.col1+col2" notation for composite keys.
    relationships: [
        {
            from: 'sales_order_headers.salesOrder',
            to: 'outbound_delivery_items.referenceSdDocument',
            label: 'FULFILLED_BY',
            joinType: 'JOIN',
            description: 'Sales Order to Delivery'
        },
        {
            from: 'outbound_delivery_items.deliveryDocument+deliveryDocumentItem',
            to: 'billing_document_items.referenceSdDocument+referenceSdDocumentItem',
            label: 'BILLED_AS',
            joinType: 'JOIN',
            description: 'Delivery to Billing'
        },
        {
            from: 'billing_document_headers.accountingDocument+companyCode+fiscalYear',
            to: 'journal_entry_items_accounts_receivable.accountingDocument+companyCode+fiscalYear',
            label: 'POSTED_AS',
            joinType: 'LEFT JOIN',
            description: 'Billing to Journal Entry'
        },
        {
            from: 'journal_entry_items_accounts_receivable.clearingAccountingDocument+companyCode',
            to: 'payments_accounts_receivable.clearingAccountingDocument+companyCode',
            label: 'CLEARED_BY',
            joinType: 'LEFT JOIN',
            description: 'Journal Entry to Payment'
        },
        {
            from: 'billing_document_headers.soldToParty',
            to: 'business_partners.customer',
            label: 'BILLED_TO',
            joinType: 'JOIN',
            description: 'Documents to Customer'
        },
    ],

    // ─── Domain Keywords (for query classification + guardrails) ─────────
    domainKeywords: [
    ...
    // ─── SAP-Specific Prompt Rules (validated, do not modify) ────────────
...
- STRICT RULE: For "delivered but not billed" queries, always join billing_document_items to outbound_delivery_items using: bdi.referenceSdDocument = odi.deliveryDocument AND bdi.referenceSdDocumentItem = odi.deliveryDocumentItem. Never use odi.referenceSdDocument for this join.`,
    ...
    // ─── SAP-Specific Few-Shot Examples (validated, do not modify) ───────
...
// Existing consumers import { domainKeywords, entities, relationships } directly.
const { domainKeywords, entities, relationshipLabels: relationships } = datasetConfig;

module.exports = { datasetConfig, domainKeywords, entities, relationships };
