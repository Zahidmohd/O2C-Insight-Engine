 ...
 * @returns {string[]}
 */
function generateDomainKeywords(tables) {
 ...
 * @param {Array<{name: string, displayName?: string}>} tables
 * @returns {string[]}
 */
function generateEntities(tables) {
    return tables.map(t => {
 ...
 *
 * @param {object} params
 * @param {string} params.name - Dataset name (snake_case)
 * @param {Array<{name, displayName, columns, primaryKey}>} params.tables
 * @param {Array<{from, to, label, joinType, description}>} params.relationships
 * @param {string} params.dataDir - Absolute path to data files
 * @returns {object} Full config for datasetValidator + loader + queryService
 */
function generateConfig({ name, tables, relationships, dataDir }) {
